...
interface Props {
  readonly x: number;
  readonly y: number;
  readonly eintraege: readonly MenueEintrag[];
 ...
 * Regel 4 haelt geteilte Pakete frei von Browser-Globals. Der Backstory
 * Creator hat deshalb eine eigene, gleich gebaute Fassung — zwei kurze
 * Dateien sind hier ehrlicher als ein Paket, das die Regel bricht.
 */
export function Kontextmenue({ x, y, eintraege, onSchliessen }: Props) {
  ...
  useEffect(() => {
    ...
    const daneben = (ereignis: PointerEvent) => {
      if (!box.current?.contains(ereignis.target as Node)) onSchliessen();
    };
    ...
    window.addEventListener('resize', onSchliessen);
    ...
    return () => {
      ...
      window.removeEventListener('resize', onSchliessen);
    ...
    <ul
      ...
      onKeyDown={(ereignis) => {
        ...
        if (ereignis.key === 'ArrowDown' || ereignis.key === 'ArrowUp') {
          ...
          const schritt = ereignis.key === 'ArrowDown' ? 1 : eintraege.length - 1;
          setAktiv((vorher) => (vorher + schritt) % eintraege.length);
          return;
        }
        if (ereignis.key === 'Enter' || ereignis.key === ' ') {
          ereignis.preventDefault();
          eintraege[aktiv]?.onWahl();
          onSchliessen();
        }
      }}
    >
      {eintraege.map((eintrag, nummer) => (
        <li key={eintrag.text}>
          <button
            type="button"
            role="menuitem"
            className={`${eintrag.gefaehrlich ? 'kontextmenue__gefaehrlich' : ''} ${
