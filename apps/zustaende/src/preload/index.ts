/**
 * Die Bruecke des Status Effect Creators.
 *
...
const api = {
  ...
  sammlung: {
    ...
    speichern: (zustand: Abgelegt, sprache: string) =>
      ipcRenderer.invoke(kanal('speichern'), zustand, sprache) as Promise<{
        ok: boolean;
        id: string;
        text: string;
      }>,
   ...
   * Abgebrochener Dateidialog: `{ ok: false, text: '' }` — kein Fehler,
...
contextBridge.exposeInMainWorld('zustaende', api);

export type ZustaendeApi = typeof api;
